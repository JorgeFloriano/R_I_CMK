<div>
    <section class="item" id="{{$i}}" name="txt{{$i}}">
        ...
        <section style="{{$disp}}" class="obs" name="txtSecJust{{$i}}" id="secJust{{$i}}">
            ...
            <textarea name="txtJust{{$i}}" {{$req}} id="idJust{{$i}}" class='autoExpand' rows='1' data-min-rows='1' placeholder="Ítens substituídos ou recuperados devem ser justificados!">{{$jus[$i] ?? ''}}</textarea>
        </section>
    </section>
</div>